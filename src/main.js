...
export default class GraphDrawer {
    ...
    #setUpListener(){
        if (this.onNodeClick){
            this.canvas.addEventListener('click', (event) => {
                let clickedOnNode = this.#posHasNode(new Vec(event.pageX- event.target.offsetLeft, event.pageY- event.target.offsetTop))
                if (clickedOnNode !== null) {
                    this.onNodeClick(clickedOnNode.name)
                }
            })
        }
        if (this.onNodeHover){
            ...
            this.canvas.addEventListener('mousemove', (event) => {
                let nodeHoverOver = this.#posHasNode(new Vec(event.pageX- event.target.offsetLeft, event.pageY- event.target.offsetTop))
                if (nodeHoverOver !== null && !onNode) {
                    this.drawCanvas.drawNode(nodeHoverOver, this.options.nodeRadiusHover)
                    onNode = true
                    this.onNodeHover(nodeHoverOver.name)
                }
                if (nodeHoverOver === null && onNode) {
    ...
    #posHasNode(pos) {
        ...
        this.canvasNodes.forEach(node => {
            if (pos.x > (node.posX - this.options.nodeRadiusHover * NODE_HOVER_RADIUS_FACTOR) && 
